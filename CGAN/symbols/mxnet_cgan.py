...
def get_symbol_generator():
    ...
    rand = mx.sym.Variable('rand')  # batch x dim x 1 x 1
    class_label = mx.sym.Variable('class_label')  # batch x 10
    class_label = mx.sym.reshape(class_label, shape=(0, 0, 1, 1))

    data = mx.sym.concat(class_label, rand, dim=1)

    g1 = mx.sym.Deconvolution(data, name='g1', kernel=(4, 4), num_filter=1024, no_bias=no_bias)
...
def get_symbol_discriminator():
    ...
    data = mx.sym.Variable('data')  # batch x 3 x 64 x 64
    label = mx.sym.Variable('label')  # batch x 1

    class_label = mx.sym.Variable('class_label')  # batch x 10
    class_label = mx.sym.reshape(class_label, shape=(0, 0, 1, 1))
    class_label = mx.sym.broadcast_to(class_label, shape=(0, 0, 64, 64))

    data_concat = mx.sym.concat(data, class_label, dim=1)

    d1 = mx.sym.Convolution(data_concat, name='d1', kernel=(4, 4), stride=(2, 2), pad=(1, 1), num_filter=128, no_bias=no_bias)
