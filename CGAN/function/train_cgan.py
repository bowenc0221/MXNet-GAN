...
def main():
    ...
    for epoch in range(config.TRAIN.end_epoch):
        ...
        for t, batch in enumerate(train_iter):
            ...
            generator.forward(mx.io.DataBatch([batch_label_one_hot]+rbatch.data, []), is_train=True)
            outG = generator.get_outputs()

            # update discriminator on fake
            label[:] = 0
            discriminator.forward(mx.io.DataBatch(outG+[batch_label_one_hot], [label]), is_train=True)
            ...
            label[:] = 1
            batch.label = [label]
            discriminator.forward(mx.io.DataBatch(batch.data+[batch_label_one_hot], [label]), is_train=True)
            discriminator.backward()
